...
def drawShape(Turtle, sides, length):
    # draws turtle with sides and length given
...
def SpinPolygon(Turtle, sides, angle, length, repeat):
    # creates a polygon and spins it by the by the intial angle calculated
    # in the function angle
...
def ScalePolygon(Turtle, sides, length, sfactor, number):
    # scales a polygon the by sfactor with amplifies the intial size and then
    # repeats the number of times it is drawn
